...
void Prueba_Noticias(){
  ...
  if(strcmp(cad3, "contenido")!=0) Ok=false;
  N.Obtener_Fecha( g);
  a=g.obtener_dia();
  b=g.obtener_anio();
  c=g.obtener_mes();
  ...
  if(strcmp(cad1,"N001")!=0)  Ok=false;
  N.Obtener_Imagen( cad2);
  if(strcmp(cad2,"imagen")!=0)  Ok=false;
  N.Obtener_SubTitulo( cad2);
  if(strcmp(cad2,"subtitulo")!=0)  Ok=false;
  N.Obtener_Titulo( cad2);
  if(strcmp(cad2,"titulo")!=0)  Ok=false;
  H.Clonar(N);
  Noticia J(H);
  if(J.Comparar( "N001")!=0)  Ok=false;
