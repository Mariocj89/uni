...
*/
const long Pausa = 100000;  // Este valor se puede aumentar o disminuir
...
bool TEntornoCargarConfiguracion(int tablero[MAX_TAMANIO][MAX_TAMANIO], int &tamanio, int sudoku){
	...
	int fila=0;
	if (sudoku==1)
		entrada.open("sudoku01.cnf");
	if (sudoku==2)
		entrada.open("sudoku02.cnf");
	if (sudoku==3)
		entrada.open("sudoku03.cnf");
	if (entrada == 0 ){
...
void TEntornoIniciarPantalla (int tablero[MAX_TAMANIO][MAX_TAMANIO], int tamanio) {
  ...
  win = newwin(36,46, 0,0);  // Ventana de juego
   ...
   msgwin = newwin(19,32,2,47);  
    ...
    mvwprintw(msgwin, 2, 8,"A - Izquierda");
    mvwprintw(msgwin, 3, 8,"D - Derecha");
    mvwprintw(msgwin, 4, 8,"W - Arriba");
    mvwprintw(msgwin, 5, 8,"S - Abajo");
    mvwprintw(msgwin, 6, 8,"P - Eliminar");
    mvwprintw(msgwin, 7, 8,"X - Salir");
    mvwprintw(msgwin, 8, 8,"R - Reiniciar");
    mvwprintw(msgwin, 9, 8,"M - Mostrar Posibles");

    mvwprintw(msgwin, 10, 4,"----------------------");

    mvwprintw(msgwin, 17, 4,"Por Mario Corchero");
...
void TEntornoMostrarMensaje2(char *c){
   mvwprintw(msgwin, 13, 5,"                          ");
   wrefresh(msgwin);
   refresh();
   mvwprintw(msgwin, 13, 5,c);
   wrefresh(msgwin);
   refresh();
}

void TEntornoMostrarMensajePosicion(char *c, int x){
   mvwprintw(msgwin, x, 5,"                          ");
   wrefresh(msgwin);
   refresh();
   mvwprintw(msgwin, x, 5,c);
   wrefresh(msgwin);
   refresh();
}

void TEntornoMostrarMensaje2Posicion(char *c, int x){
   mvwprintw(msgwin, 14, x,"                       ");
   wrefresh(msgwin);
   refresh();
   mvwprintw(msgwin, 14, x,c);
   wrefresh(msgwin);
   refresh();
}
				...
				switch (extendido)
 					{
						...
						case 'j' : tecla=TJugar;
								 ...
								 break;
						case 'R' :
						case 'r' : tecla=TReiniciar;
								 salir=true;
								 break;
						case 'M' :
						case 'm' : tecla=TMostrar;
								 salir=true;
								 break;
