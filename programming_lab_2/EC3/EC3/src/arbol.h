 ...
 */
#define kDepurarArbol 0

/*!
   \def DEPURAR_ARBOL_MSG
   Macro utilizada paa mostrar mensajes de depuración de programa y retener los mensajes en pantalla
 */
#define DEPURAR_ARBOL_MSG(msg) if (kDepurarArbol){ cout<<msg; }
...
public:
 ...
 private:
  ...
  bool Iguales(const Arbol *A) const;
};
...
Arbol <T>::Arbol()
{
    DEPURAR_ARBOL_MSG ("Entra en Arbol::Arbol"<<endl);
    vacio_ = true;
    hizq_ = NULL;
    hder_ = NULL;
    DEPURAR_ARBOL_MSG ("Sale de Arbol::Arbol"<<endl);
}
...
{
    DEPURAR_ARBOL_MSG ("Entra en Arbol::Arbol(Arbol*,DatoARbol&,Arbol*): "<<dato<<endl);
    ...
    this->vacio_=false;
    DEPURAR_ARBOL_MSG ("Sale de Arbol::Arbol(Arbol*,DatoARbol&,Arbol*)"<<endl);
}
 ...
 * @return numero de nodos que tiene el nivel
 */
template <class T>
...
{
    DEPURAR_ARBOL_MSG ("Entra en Arbol::Insertar(DatoARbol&): "<<dato<<endl);
    bool resultado=true;
    if (vacio()) {
        dato_raiz_ = dato;
		vacio_ = false;
    ...
    DEPURAR_ARBOL_MSG ("Sale de Arbol::Insertar(DatoARbol&)"<<endl);
    return resultado;
}
...
{
    DEPURAR_ARBOL_MSG ("Entra en Arbol::pertenece(DatoARbol&): "<<dato<<endl);
    Arbol *aux;
    ...
    DEPURAR_ARBOL_MSG ("Sale de Arbol::pertenece(DatoARbol&)"<<endl);
    return encontrado;
}
...
{
    DEPURAR_ARBOL_MSG ("Entra en Arbol::borrar(DatoARbol&): "<<dato<<endl);
    if (!vacio())
    {
    ...
    DEPURAR_ARBOL_MSG ("Sale de Arbol::borrar(DatoARbol&)"<<endl);
}
...
{
    DEPURAR_ARBOL_MSG ("Entra en Arbol::borrarOrden(DatoARbol&): "<<dato<<endl);
    T datoaux;
    ...
    DEPURAR_ARBOL_MSG ("Sale de Arbol::borrarOrden(DatoARbol&)"<<endl);
    return retorno;
}
 ...
 * @param A segundo elemento a comparar
 * @return bool si son iguales, false en caso contrario
 */
template <class T>
bool Arbol <T>::operator==(const Arbol &A) const {
  return this->Iguales(&A);
}

/**
 * Sobrecarga del operador !=
 * @param A segundo elemento a comparar
 * @return bool si no son iguales, false en caso contrario
 */
template <class T>
bool Arbol <T>::operator!=(const Arbol &A) const {
  return !this->Iguales(&A);
...
/**
 * Funcion auxiliar para la sobrecarga del operador ==
 * @param A segundo parametro a comparara
 * @return true si los 2 arboles son iguales, false en caso contrario
 */
template <class T>
bool Arbol <T>::Iguales(const Arbol *A) const {
  if (A==NULL && this==NULL) return true;//caso base, ambos son null, luego son iguales
  if (A==NULL || this==NULL) return false;//caso base, uno es null, luego son distintos
  if (A->dato_raiz_ != this->dato_raiz_) return false; //comprobamos la raiz
  else if (A->hizq_->Iguales(this->hizq_) && A->hder_->Iguales(this->hder_)) return true; //comprobamos recursivamente los hijos
  return false;
}
...
{
    DEPURAR_ARBOL_MSG ("Entra en Arbol::Destruir()"<<endl);
    Arbol *aux;
    if (!vacio())
    {
        if ((aux=hijoIzq())!=NULL)
            {aux->Destruir();delete hizq_;hizq_=NULL;}
        if ((aux=hijoDer())!=NULL)
            {aux->Destruir();delete hder_;hder_=NULL;}
        delete dato_raiz_;
        dato_raiz_=NULL;
    }
    DEPURAR_ARBOL_MSG ("Sale de Arbol::Destruir()"<<endl);
}
...
/**
 * devuelve un flujo con los datos del arbol a traves de un recorrido inorden
 * @return flujo a devolver
 */
template <class T>
void Arbol< T * >::ACola(queue<T*> &inorden) const
{
  if(!this->vacio()){
    this->hijoIzq()->ACola(inorden);
    inorden.push(dato_raiz_);
    this->hijoDer()->ACola(inorden);
  }
}
