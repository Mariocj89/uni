...
void Cargador::crear(string elto, int numCampos, string vCampos[kMaxCampos])
{
    ...
    if (numElto!=-1 && mapeo[numElto].numCampos==numCampos)
    {
        ...
        case 2:
            ...
            break;
        case 3:
            crearPresoOptimo(numCampos,vCampos);
            break;
     	}
    }
    else
...
/**
 * Metodo que crea un PresoOptimo y lo inserta en la prision
* \param numCampos número de campos ocupados del vector vCampos
* \param vCampos[] valores de configuración del elemento a crear
* \return No devuelve nada
 */
void Cargador::crearPresoOptimo(int numCampos, string vCampos[])
{
    DEPURAR_MSG("Entra en Cargador::crearPresoOptimo ("<<numCampos<<")");
    cout << "Creado PresoOptimo: " << vCampos[1] << endl;
    Posicion orientacion[4];
    for(int i = 0;i < 4;i++)
      switch(vCampos[2][i]){
        case 'N' : orientacion[i] = NORTE; break;
        ...
        case 'E' : orientacion[i] = ESTE; break;
        case 'O' : orientacion[i] = OESTE; break;
        default : throw string("Atencion, error en la ruta de un OresoOptimo, revise el fichero inicio.txt");
      }
    Prision::get_instancia()->set_persona(Prision::get_instancia()->get_numero_plantas()-1,0,new PresoOptimo(vCampos[0],vCampos[1][0],atoi(vCampos[3].c_str()),orientacion,Prision::get_instancia()->get_numero_plantas()-1,0));
    Prision::get_instancia()->inc_numero_presos();
    DEPURAR_MSG("Sale de Cargador::crearPresoOptimo");
}
