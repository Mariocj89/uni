...
Puerta::~ Puerta()
{
  ...
  if(copia_combinacion_!=NULL) {
    while(!copia_combinacion_->vacia()) {
      delete copia_combinacion_->get();
      copia_combinacion_->borrar();
    }
    delete copia_combinacion_;
  }
}
...
{
  queue <Llave*> cola;
  P.probadas_->ACola(cola);
  flujo<<"(puerta:";
  switch(P.estado_){
    case NOCONFIGURADA: flujo<<"no configurada"; break;
    case CERRADA:  flujo <<"cerrada"; break;
    case ABIERTA:  flujo <<"abierta"; break;
  }  
  flujo<<":"<<P.altura_<<": ";
  while(!cola.empty()){
    flujo<<" "<<*cola.front();
